// src/pages/students/add-student.tsx
...
const AddStudentPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await axios.post(`${apiUrl}/api/students`, payload);
    ...
    <div className="flex flex-col h-screen">
      ...
      <main className="flex flex-1 overflow-y-auto bg-gray-100 p-6">
        <section className="bg-white max-w-2xl w-full mx-auto p-8 rounded-lg shadow-md">
          <h1 className="text-2xl font-semibold mb-6 text-gray-800 text-center">Adicionar Aluno</h1>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <button
              type="submit"
              disabled={loading}
              className={`w-full py-3 rounded-md text-white font-semibold ${loading ? 'bg-green-300 cursor-not-allowed' : 'bg-green-600 hover:bg-green-700'} transition`}
            >
              {loading ? 'Salvando...' : 'Adicionar Aluno'}
            </button>
          </form>
        </section>
      </main>
